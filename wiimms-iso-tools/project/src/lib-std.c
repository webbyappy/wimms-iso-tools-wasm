...
void SetupLib ( int argc, char ** argv, ccp p_progname, enumProgID prid )
{
    ...
    #ifdef WIIMM_TRUNK
	revision_id = (prid << 20) + SYSTEMID + 0 + REVID_WIIMM_TRUNK;
    #elif defined(WIIMM)
	revision_id = (prid << 20) + SYSTEMID + 0 + REVID_WIIMM;
    #else
	revision_id = (prid << 20) + SYSTEMID + 0 + REVID_UNKNOWN;
    #endif
